...
function MainBackgroundColoredGlobalComponent({title, subTitle, buttonLink, buttonLabel='EN APPRENDRE PLUS', img}: Props) {
  return (
    <motion.div whileInView={{ opacity: 1 }} initial={{ opacity: 0 }} className=" h-screen flex flex-col justify-center items-center lg:flex-row bg-tertiary dark:bg-gray-900 lg:my-10 lg:h-[100vh] overflow-y-hidden">
      <div className="container px-6 mx-auto lg:h-full">
        <div className="items-center justify-center flex-col  lg:h-full">
          ...
          <motion.div whileInView={{ opacity: 1 }} initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1 }} className="flex items-center justify-center w-full mt-6 lg:w-full">
            <Image className="w-full h-2/3 lg:max-w-3xl lg:h-3/4 shadow-xl" src={img} alt="whiteboard" />
          </motion.div>
...
export default MainBackgroundColoredGlobalComponent